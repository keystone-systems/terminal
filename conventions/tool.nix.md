...
22. Keystone secret management MUST distinguish at least four classes of sops secrets (see `secrets.md` for the full secrets convention): os-level secrets, service secrets, user-home secrets, and custom secrets.
...
27. The corresponding `keystone.secrets.provided.<name>` runtime declaration MUST exist on every host where that Home Manager user is installed when the user-home profile expects `/run/secrets/<name>` to exist there.
...
29. Secret-backed environment variables in Home Manager MUST NOT be set with `home.sessionVariables` when the value would be embedded in the Nix store. Non-secret values MAY use `home.sessionVariables`, but secret values MUST be read from runtime files such as `/run/secrets/<name>` in a shell init hook or another runtime-only execution path.
30. When a tool needs both a public endpoint and a secret credential, the public endpoint SHOULD be set declaratively with `home.sessionVariables`, while the secret credential SHOULD be exported from the runtime secret file at shell startup.
31. Secret-management surfaces such as Walker menus or the `ks secrets` commands SHOULD expose os-level secrets, service secrets, user-home secrets, and custom secrets as first-class categories so users can discover the intended scope before editing recipients or values.
...
34. The module auto-discovers the secret at module-eval time when `tokenFile` is unset; explicit `tokenFile = "..."` always overrides. Discovery order:
    1. `/run/secrets/nix-github-token` — dedicated nix-daemon secret if `keystone.secrets.provided."nix-github-token"` is declared.
    2. `/run/secrets/${adminUsername}-github-token` — user-home PAT shared at os-level if `keystone.secrets.provided."${adminUsername}-github-token"` is declared.
    3. (nothing found) — module stays inert. No assertion failure, no systemd unit emitted.
35. The **preferred default** is shape (2): single PAT backing `gh` / `git` / nix-daemon. CLAUDE.md rule 26 already requires user-home secrets to include every relevant host's system key, so root on those hosts can already decrypt the same ciphertext. Adopters declare one `keystone.secrets.provided."${username}-github-token"` with `owner = "root"; group = "users"; mode = "0440";` — root reads it for the nix-daemon include file, the `users` group reads it for the user shell env hook.
...
```nix
# secrets/recipients.nix — grant the consuming hosts access to shared.yaml
files."secrets/shared\\.yaml".hosts = [ "ncrmro-workstation" "ncrmro-laptop" ];

# nixos host that runs `ks update` / `nix flake update`
# (key ncrmro-github-token lives in secrets/shared.yaml)
keystone.secrets.provided."ncrmro-github-token" = {
  owner = "root";
  group = "users";
  mode = "0440";
  scope = "shared";
...
# tokenFile auto-resolves to /run/secrets/ncrmro-github-token via the fallback chain
```

### Golden example: dedicated nix-daemon shape

```nix
# key nix-github-token lives in secrets/shared.yaml
keystone.secrets.provided."nix-github-token" = {
  owner = "root";
  mode = "0400";
  scope = "shared";
};
...
keystone.os.githubTokenNix.enable = true;
# tokenFile auto-resolves to /run/secrets/nix-github-token (preferred over user-PAT when both declared)
```

### Darwin (per-user nix.conf)

On Darwin keystone hosts there is no nix-darwin system module — Darwin keystone hosts run standalone home-manager only, and `nix flake update` is always user-invoked (no `sudo nixos-rebuild`). Use `keystone.terminal.githubTokenNix` to materialize `~/.config/nix/access-tokens.conf` at home-manager activation. The default `source = "gh-auth"` shells out to `gh auth token` and requires no secrets wiring. For adopters with a Darwin secrets backend, set `source = "tokenFile"; tokenFile = <path to the decrypted token>;` to share the same PAT used elsewhere.

```nix
# Darwin default — uses gh CLI login
keystone.terminal.githubTokenNix.enable = true;

# Darwin with a locally materialized token file
keystone.terminal.githubTokenNix = {
  enable = true;
  source = "tokenFile";
  tokenFile = "/run/secrets/ncrmro-github-token";
};
```
