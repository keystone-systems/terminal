...
When invoked as `$ks <route>`, this skill gives Keystone help for that route.
It MUST NOT execute the similarly named `ks` CLI command without explicit
authorization.

## Session context

- Capabilities: __CAPABILITIES__
- Development mode: __DEVELOPMENT_MODE__
- Published commands: __PUBLISHED_COMMANDS__
...
- `$ks doctor`: diagnose the reported Keystone problem.
- `$ks issue`: help draft and file a Keystone issue.
