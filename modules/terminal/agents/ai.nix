...
in
{
  ...
  config = mkIf (cfg.enable && cfg.ai.enable) {
    home.packages = [
      ...
      pkgs.keystone-terminal.claude-code

      # Gemini CLI - Google's AI assistant
      pkgs.keystone-terminal.gemini-cli

      # Codex - OpenAI's lightweight coding agent
      pkgs.keystone-terminal.codex

      # OpenCode - Open-source AI coding agent
      pkgs.keystone-terminal.opencode
    ...
    xdg.configFile."keystone/shell.d/20-ollama.zsh" = mkIf ollamaCfg.enable {
      text =
        let
