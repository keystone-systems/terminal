...
- `keystone-sync-agent-assets` regenerates keystone-curated content. Per-host
...
`AGENTS.md` and `conventions/` are regenerated by `keystone-sync-agent-assets`.
...
`keystone-sync-agent-assets`. To change a convention, edit the keystone repo
...
`keystone-sync-agent-assets` script does not generate persona files; it
